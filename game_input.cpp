    ...
    if(in_progress){
        if(network.status=="server"){
            ...
            for(int i=0;i<network.clients.size();i++){
                if(!paused){
                    if(i<2){
                        if(network.clients[i].game_command_state("up")){
                            paddles[i].velocity.direction=90.0;
                            paddles[i].velocity.magnitude=144.0;
                        }
                        else if(network.clients[i].game_command_state("down")){
                            paddles[i].velocity.direction=270.0;
                            paddles[i].velocity.magnitude=144.0;
                        }
                        else{
                            paddles[i].velocity.direction=0.0;
                            paddles[i].velocity.magnitude=0.0;
                        }
                    }
                }
            }
        ...
        if(network.status=="server"){
            for(int i=0;i<network.clients.size();i++){
                for(int j=0;j<network.clients[i].command_buffer.size();j++){
                    ...
                    if(!paused){
                        if(i<2){
                            if(paddles[i].is_interactive()){
                                if(command_name=="use_ability"){
                                    if(paddles[i].is_cooled()){
                                        paddles[i].use_ability(i);
                                    }
                                }
                            }
                        }
    ...
    if(in_progress){
        if(!paused){
            if(engine_interface.game_command_state("up")){
                command_states.push_back("up");
            }

            if(engine_interface.game_command_state("down")){
                command_states.push_back("down");
            }
        }
    }
}
