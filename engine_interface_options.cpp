        ...
        while(!load.eof()){
            ...
            //Load data based on the line.
                ...
                if(command_parts.size()==7){
                    ...
                    game_commands[game_commands.size()-1].name=command_parts[0];
                    game_commands[game_commands.size()-1].title=command_parts[1];
                    game_commands[game_commands.size()-1].description=string_stuff.process_newlines(command_parts[2]);
                    game_commands[game_commands.size()-1].dev=string_stuff.string_to_bool(command_parts[3]);
                    game_commands[game_commands.size()-1].key=key;
                    game_commands[game_commands.size()-1].button=button;
                    game_commands[game_commands.size()-1].axis=axis;
                }
            }
