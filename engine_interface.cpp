...
void Engine_Interface::load_game_constant(File_IO_Load* load){
    game_constants.push_back(Game_Constant());

    string constant_default="";
    ...
    while(!load->eof()){
        ...
        //Name
            ...
            game_constants[game_constants.size()-1].name=line;
        }
        //Default
        else if(!multi_line_comment && boost::algorithm::starts_with(line,str_default)){
            //Clear the data name.
            ...
            constant_default=line;
        }

        //If the line ends the game constant.
        else if(!multi_line_comment && boost::algorithm::starts_with(line,"</game_constant>")){
            if(constant_default.length()>0){
                game_constants[game_constants.size()-1].set_value(constant_default);
            }
...
Game_Constant* Engine_Interface::get_game_constant(string name){
    Game_Constant* ptr_object=0;

    for(int i=0;i<game_constants.size();i++){
        if(game_constants[i].name==name){
            ptr_object=&game_constants[i];
    ...
    if(ptr_object==0){
        message_log.add_error("Error accessing game constant '"+name+"'");
    }
...
void Engine_Interface::animate(){
    ...
    for(int x=0;x<game.menu_tiles.size();x++){
        for(int y=0;y<game.menu_tiles[x].size();y++){
            game.menu_tiles[x][y].animate(true);
        }
    }
}
